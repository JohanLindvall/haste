...
- **The XXH64 kernels are the ones that reference a Go symbol.** Generated
  ...
  - **amd64** loads four of the primes into registers RIP-relative and keeps
    no pointer at all. A pointer costs 12-16% of a 32..256-byte hash there;
    see the XXH64 performance notes. P3 has no register left and is a
    `movabs` at its two cold uses.

  `primes` in `xxh64.go` is therefore read-only and must stay `[6]uint64` in
  that order, the sixth slot being arm64's lane-round form. A backend that
  wants constants in registers implements `TableLoader`; one that wants a
  pointer sets `TableGPR`. Both go through `FuncDef.Table`, and
  `asmgen.PrologueLoads` reports the register form so the simulator test can
  set up the same state -- the prologue is not part of the instruction
  stream, so without that the simulated kernel would run on zeroed primes.
...
- **amd64 is one form** and imul-bound: eight `imul` per block at one per
  ...
  chain, and cespare's kernel is already there.
- **Never reach the primes through a pointer on amd64.** Measured on a
  Redwood Cove (Core Ultra 9 185H). The kernel used to load the table's
  address with `LEAQ ·primes(SB), CX` and read the primes off `CX`; it now
  loads four of them into registers RIP-relative and keeps no pointer.
  Cycles per hash, and what it was against cespare/xxhash:

  | bytes | before | after | cespare |
  |---|---|---|---|
  | 32 | 26.7 | 23.3 | 22.6 |
  | 64 | 36.0 | 30.8 | 30.4 |
  | 128 | 55.2 | 46.7 | 46.5 |
  | 256 | 87.7 | 79.4 | 78.8 |
  | 1024 | 282.3 | 273.9 | 273.1 |

  −12.7% at 32 bytes rising to −15.4% at 128, and level with cespare from
  32 bytes up where it had been 15% behind.

  The cause was isolated the other way round, by adding pieces of this kernel
  to a copy of cespare's and taking nothing else away. A table pointer
  reproduced the whole gap: 30.4 to 34.9 cycles at 64 bytes, 46.8 to 55.0 at
  128. Seeded lane initialization, xxhaste's five-way tail branch chain and
  P4 as a memory operand each cost nothing. From this side the same: a
  cheaper tail, a single-block loop and seed-free lanes removed up to twelve
  instructions per hash and moved not one cycle.

  **The mechanism is not understood, and the shape of it is strange enough to
  record.** What costs the ~6.5 cycles is the presence of a RIP-relative
  `lea` in the kernel, not the loads it feeds: adding a *dead* one to
  cespare's kernel costs the same 6.5 cycles, a second one costs nothing more,
  and `lea 8(%rsp), %rcx` or `movq ·primes(%rip), %rcx` in the same place cost
  nothing at all. It is not code alignment -- both kernels measure flat across
  all eight 8-byte offsets within a 64-byte line -- not the front end
  (fe-bound stays at 1%), and not speculation (no machine clears). It is
  core-bound, and it reproduces on all three P-cores. Note that a
  RIP-relative `lea` of `kSecret` sits in every XXH3 hash's call path and
  costs nothing there, so whatever this is, it is not simply "that
  instruction is slow". Beware of generalizing it; the actionable part is the
  measurement above.
...
- Streaming was profiled rather than guessed at, and the guess was wrong: for
  ...
  3. The staging size went from the reference's 256 bytes to 512. It is a
     ...
     **It is 1024 now**, raised on a Redwood Cove where 1024 was better or
     ...
     originally recorded against going past 512 -- but that measurement was
     made on the N2 and has not been repeated. **If an N2 is at hand, measure
     512 against 1024 there before trusting this value on arm64.**
...
- An exhaustive per-length sweep (bench/sweep, every length 0..255, both
  ...
  **`bench/sweep` used to time one hash at a time through a `func([]byte)
  uint64`, and that indirection is not neutral between implementations.** It
  cost xxhaste 0.65ns per hash and zeebo/xxh3 0.15ns on a Redwood Cove --
  these entry points are shaped to inline into their caller and reach the
  kernel in one call, so an indirect call takes that away and adds a level.
  It was enough to invert the comparison: the sweep reported xxhaste 9%
  behind over 33..64 bytes where a direct call has it 6% ahead. Each
  implementation now owns its whole iteration loop, so the hash inside is
  direct. Numbers from a sweep run before that change are not comparable with
  the ones above; re-run it rather than trusting them.
