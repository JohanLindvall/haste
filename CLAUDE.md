...
- **XXH64's arm64 kernel reads a table.** It is the one kernel that references
  a Go symbol: the prologue (Go assembly text, not generated bytes) loads
  `$·primes(SB)` into a register the generated code then reads five words
  from, because five 64-bit immediates would be four instructions each in
  code with no constant pool. `primes` in `xxh64.go` is therefore read-only
  and must stay `[5]uint64` in that order. The x86 kernel uses `movabs`. Any
  further table goes through `FuncDef.Table` and `TableGPR`, so the simulator
  test can place it too.
