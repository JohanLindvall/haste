...
func len1to3_128(in unsafe.Pointer, n uintptr, sec unsafe.Pointer, seed uint64) Uint128 {
	c1 := uint32(rdb(in, 0))
	...
	bitflipl := uint64(rd32(sec, 0)^rd32(sec, 4)) + seed
	bitfliph := uint64(rd32(sec, 8)^rd32(sec, 12)) - seed
	return Uint128{
...
func len4to8_128(in unsafe.Pointer, n uintptr, sec unsafe.Pointer, seed uint64) Uint128 {
	seed ^= uint64(bits.ReverseBytes32(uint32(seed))) << 32
	inputLo := rd32(in, 0)
	inputHi := rd32(in, n-4)
	input64 := uint64(inputLo) + uint64(inputHi)<<32
	bitflip := (rd64(sec, 16) ^ rd64(sec, 24)) + seed
	keyed := input64 ^ bitflip

	// The length is shifted left so the multiplier is always even, which keeps
	// the low half from losing bits to a power-of-two factor.
...
func len9to16_128(in unsafe.Pointer, n uintptr, sec unsafe.Pointer, seed uint64) Uint128 {
	...
	// Placing the length mid-word ensures it reaches both halves of the
...
func len1to3_128NS(in unsafe.Pointer, n uintptr, sec unsafe.Pointer) Uint128 {
	c1 := uint32(rdb(in, 0))
	c2 := uint32(rdb(in, n>>1))
	c3 := uint32(rdb(in, n-1))
	combinedl := c1<<16 | c2<<24 | c3 | uint32(n)<<8
...
func len4to8_128NS(in unsafe.Pointer, n uintptr, sec unsafe.Pointer) Uint128 {
	inputLo := rd32(in, 0)
	inputHi := rd32(in, n-4)
	input64 := uint64(inputLo) + uint64(inputHi)<<32
	keyed := input64 ^ (rd64(sec, 16) ^ rd64(sec, 24))
...
func len9to16_128NS(in unsafe.Pointer, n uintptr, sec unsafe.Pointer) Uint128 {
