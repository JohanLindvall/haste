...
func simBlocks(t *testing.T, k asmgen.Kernel, lanes *[4]uint64, in []byte, nb int, split int) {
	t.Helper()
			...
			t.Run(fmt.Sprintf("%s-%s/split%d", b.Name, b.GOARCH, split), func(t *testing.T) {
				...
				for _, n := range lens {
					for _, seed := range []uint64{0, 1, 0x9E3779B185EBCA87, ^uint64(0)} {
						if got, want := simSum64(t, sum64, buf[:n], seed, split), sum64Generic(ptr(buf), n, seed); got != want {
				...
				for _, v := range refVecs {
					...
					if got := simSum64(t, sum64, buf[:v.Len], v.Seed, split); got != v.H64 {
				...
				for i := 0; i < 200; i++ {
					...
					w = v
					nb := rng.Intn(20)
					simBlocks(t, blocks, &v, buf[:nb*blockLen], nb, split)
